...
import static view.Farming.items;
import static view.Farming.getTractorPrice;
import static view.Farming.getPricingFactor;
import static view.Farming.getIrrigationPrice;
import static view.Farming.getTax;
import static view.Farming.setTractorBuy;
import static view.Farming.setIrragation;
import static view.Farming.isFirst;
import static view.Farming.setFirst;
...
public class MarketController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (isFirst()) {
            setFirst(false);
            items.add(new Item("Tractor", getPricingFactor() * (getTractorPrice() + getTax())));
            items.add(new Item("Irrigation", getPricingFactor() * (getIrrigationPrice() + getTax())));
        }
        setMoneyText();
...
//        sellAll();
    ...
    void sellAll() {
        ...
        ObservableList<Crop> crops = Inventory.crops;
        for (Crop crop : crops) {
            try {
                Farming.setTotalMoney(Farming.getTotalMoney() + crop.getPrice() * crop.getQuantity());
//                inventoryTable.getItems().remove(crop);
                money.setText("Money: " + Farming.getTotalMoney());
                Inventory.setCapacity(Inventory.getCapacity() + crop.getQuantity());
                capacity.setText("Capacity: " + (Inventory.getTotal() - Inventory.getCapacity()) + "/" + Inventory.getTotal());
            } catch (Exception ex) {
    ...
    private void buy(ActionEvent event) {
        ...
        if (itemR.isSelected()) {
            ...
            if (selectedItem.getName().equalsIgnoreCase("Tractor")) {
                setTractorBuy(true);
            }
            if (selectedItem.getName().equalsIgnoreCase("Irrigation")) {
                setIrragation(true);
            }
